...
public class moneyTax {
	...
	public static void main(String[] args) {
		...
		int k20_tax = taxcal(k20_val, k20_rate);	//함수 taxcal을 사용하고 위에 값 대입
	...
	//static 메모리에 저장함
	//없으면 함수가 호출 될 때만 저장 
	public static int taxcal(int k20_val, int k20_rate) {	//정수형 값이 들어가는 함수 선언 
		int ret20;	//정수혀 함수 선언
		
		if(((double)k20_val * (double)k20_rate / 100.0) == k20_val * k20_rate / 100) {	
			//실수형인 val과 rate / 100가 정수형 val * k20/100과 같으면
			ret20 = k20_val * k20_rate / 100;	//ret20은 소수점 아래를 버린 정수값
		} else {	//위의 조건식이 아니면
			ret20 = k20_val * k20_rate / 100 + 1;	//소수점아래를 버리고 1을 더함
		}
		return ret20;	//값을 반환
	}
